...
class Trainer:
    ...
    def run(self):
        ...
        for i, timestep in enumerate(timesteps):
            ...
            xt_tmp =  coe["at"] * xt + coe["bt"] * vt +  coe["ct"]* et_zerothorder


            # Evaluation
            images = self.latents_to_images(xt_tmp)
            rewards = self.reward_fn(images, [self.prompt]*xt_tmp.shape[0]).to(self.accelerator.device)
            ...
            xt =  coe["at"] * xt + coe["bt"] * vt + GC * coe["ct"]* zt
        ...
        ImageRecords = './results/evolvable_' +  str(self.config.reward) + '/Evolvable_' + self.prompt   +  '_' + str(Inference_N_step)  + '_' + str(self.config.diffusion.guidanceC) + '_' + str(Eva_rewards[0].data.cpu().numpy()) +  '_' + str(self.config.seed) + '.png'
        
        
        final_images[0].save(ImageRecords)

        saveRewards = './results/evolvable_' +  str(self.config.reward) + '/Evolvable_' + self.prompt  + '_' + str(Inference_N_step) + '_' + str(self.config.diffusion.guidanceC)  +   '_' + str(self.config.seed) + '.npz'
        np.savez(saveRewards,array1 = Iter_Reward)  

        
        
        txtfile = './results/evolvable_' +  str(self.config.reward) + '/Evolvable_' + self.prompt  + '_' + str(Inference_N_step) + '_' + str(self.config.diffusion.guidanceC)  + '_' + str(self.config.diffusion.noise_schedule) + '.txt'
