...
class Trainer:
    ...
    def run(self):
        ...
        output_dir = Path("./final_results11") / f"StableDiff_FlowGRPO_DefaultSampler_{self.config.reward}"
        output_dir.mkdir(parents=True, exist_ok=True)

        ImageRecords = output_dir / ('StableDiff_Zeroth_' + self.prompt   +  '_' + str(Inference_N_step)  + '_' + str(self.config.diffusion.guidanceC) + '_' + str(Eva_rewards[0].data.cpu().numpy()) +  '_' + str(self.config.seed) + '.png')
        
        
        final_images[0].save(ImageRecords)

        saveRewards = output_dir / ('StableDiff_Zeroth_' + self.prompt  + '_' + str(Inference_N_step) + '_' + str(self.config.diffusion.guidanceC)  +   '_' + str(self.config.seed) + '.npz')
        
        np.savez(saveRewards,array1 = Iter_Reward)  

        txtfile = output_dir / ('StableDiff_Zeroth_' + self.prompt  + '_' + str(Inference_N_step) + '_' + str(self.config.diffusion.guidanceC)  + '_' + str(self.config.diffusion.noise_schedule) + '.txt')
