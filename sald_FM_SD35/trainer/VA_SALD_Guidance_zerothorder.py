...
FLAGS = flags.FLAGS
config_flags.DEFINE_config_file("config", "config/VA_SALD_Guidance_zerothorder.py", "Configuration.")
...
class Trainer:
    ...
    def run(self):
        ...
        output_dir = Path("./results") / f"VA_SALD_{self.config.reward}"
        output_dir.mkdir(parents=True, exist_ok=True)

        ImageRecords = output_dir / ('VA_SALD_Guidance_zerothorder_' + self.prompt   + '_' + str(r)  + '_' + str(K) + '_' + str(eta0)  + '_' + str(GC) + '_' + str(Iter_Reward[k]) +  '_' + str(self.config.seed) +'.png')
        
        final_images[0].save(ImageRecords)

        saveRewards = output_dir / ('VA_SALD_Guidance_zerothorder_' + self.prompt  + '_' + str(r) + '_'   + '_' + str(self.config.seed) + '.npz')
        
        np.savez(saveRewards,array1 = Iter_Reward)  

        
        txtfile = output_dir / ('VA_SALD_Guidance_zerothorder_' + self.prompt   + '_' + str(r)  + '_' + str(K) + '_' + str(eta0)  + '_' + str(GC) + '.txt')
